...
open_addr_t* neighbors_getAddress(uint8_t index) {
    return &neighbors_vars.neighbors[index].addr_64b;
}

asn_t neighbors_getASN(uint8_t index) {
    return neighbors_vars.neighbors[index].asn;
}
/**
    ...
    for (i=0;i<MAXNUMNEIGHBORS;i++) {
        if (isThisRowMatching(l2_dest,i)) {
            ...
            if (sentOnTxCell) {
                ...
                // if i am a patient, i cannot change my rank
                // if(idmanager_getMyID(ADDR_64B)->addr_64b[7] != 0x33)
