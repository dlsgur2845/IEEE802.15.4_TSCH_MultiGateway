...
void schedule_startDAGroot(void)
{
    ...
    schedule_setFrameNumber(SCHEDULE_MINIMAL_6TISCH_DEFAULT_SLOTFRAME_NUMBER);
    temp_neighbor.type = ADDR_ANYCAST;
    for(running_slotOffset = start_slotOffset; running_slotOffset <  MAXACTIVESLOTS; running_slotOffset++) {
        schedule_addActiveSlot(
            running_slotOffset,
            CELLTYPE_TXRX,
            TRUE,
...
void schedule_syncSlotOffset(slotOffset_t targetSlotOffset)
{
    ...
    for (i = 0; i < MAXACTIVESLOTS; i++)
    {
        if (schedule_vars.currentScheduleEntry->slotOffset == targetSlotOffset)
        {
            break;
        }
        schedule_advanceSlot();
